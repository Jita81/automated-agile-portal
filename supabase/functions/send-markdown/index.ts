...
const MARKDOWN_CONTENT = `# Automated Agile — Process Architecture
...
## 02 — The Decision Taxonomy

### Twelve Decision Types
...
**D7 — Context Package Approval** *(Critical Gate)*
...
> **The Critical Gate:** Decision 7 is the most important gate in the entire process. Nothing enters the manufacturing pipeline without a signed-off context package. This is where quality is assured — not after code is generated, but before.

#### Delivery Decisions — Per Sprint / Per Story

**D8 — Sprint Commitment**
*"Which stories do we commit to this sprint?"*
- **Inputs:** Context readiness scores for all candidate stories, team capacity, dependency status, prior sprint velocity.
...
- **Inputs:** Approved context package, current codebase snapshot, manufacturing configuration, predicted queue outcome.
...
- **Inputs:** Generated code, test results, AI self-assessment, acceptance criteria from context package, coding standards.
...
- **Inputs:** Triage feedback from current sprint, queue distribution trends, differential analysis of Q2 code, Q3 failure patterns.
...
- **Inputs:** All stories in phase: acceptance criteria pass/fail, test coverage, queue distribution, defect count, performance benchmarks, stakeholder demo feedback.
...
**Before — Agenda Generation:** The platform scans the Context Graph for gaps relevant to the meeting's purpose and participants. It produces a structured agenda of specific questions that need answers, sent to participants in advance.

**During — Real-Time Sufficiency:** The platform processes the transcript in real time and provides a live sufficiency dashboard. Green: covered. Amber: partially addressed. Red: not yet discussed. The facilitator uses this to steer conversation.

**After — Extraction & Sign-Off:** Within minutes of the meeting ending, the platform produces a Meeting Extraction Document. Automated extraction → facilitator review → participant sign-off. No item enters the Context Graph without human confirmation.
...
- **Real-time:** Per-story context completeness tracker. Alerts when a story has been discussed for >15 minutes without any testable acceptance criteria stated.
...
- **Real-time:** Running capacity tracker. Alerts when committing a story with context readiness below 70%. Highlights dependency chains.
...
- **Real-time:** Sprint burndown with context readiness overlay — not just "work remaining" but "context-ready work remaining".
...
**01 — Gap Detection**
The Context Graph continuously evaluates completeness for every active work item. When a section falls below threshold (configured per team), a context gap is created automatically.

**02 — Resolution Routing**
Each gap is classified by type and routed to the appropriate resolution mechanism. A missing business rule goes to the stakeholder meeting agenda. A missing code pattern triggers Codebase Intelligence analysis.

**03 — Meeting Curation**
The Process Engine aggregates pending gaps by meeting type and schedules or adds to the next appropriate meeting. If enough high-severity gaps accumulate, it can trigger an ad-hoc session.

**04 — Resolution Capture**
When a meeting occurs, the extraction pipeline processes the transcript and marks gaps as resolved — pending human confirmation.

**05 — Quality Feedback**
When manufactured code is triaged, the triage feedback reveals whether gaps were truly resolved or merely appeared to be. Q2 and Q3 outcomes may reopen gaps that were marked resolved.
...
- **Success Patterns** — By analysing which code patterns correlate with clean PR approvals, high test coverage, and production stability, the engine builds a pattern library automatically. Developers validate these in Pattern Review meetings.
- **Anti-Patterns** — By analysing which patterns correlate with production incidents, high defect rates, or repeated reviewer comments, the engine identifies patterns to avoid. These become hard constraints in context packages.
- **Change Impact Maps** — By analysing historical co-change patterns (files that tend to change together), the engine predicts the blast radius of a modification. This feeds the "affected components" section of every context package.
...
## 06 — Output & Sign-Off Matrix

### Every Document. Every Gate.
...
*Learn more at [contextengineering.team](https://contextengineering.team/) · [softwaremanufacturing.team](https://softwaremanufacturing.team/)*
...
<head>
  ...
  <title>Automated Agile — Process Architecture</title>
</head>
<body style="font-family: 'Georgia', serif; background-color: #0a0a0a; color: #e8e0d4; margin: 0; padding: 40px 20px;">
  <div style="max-width: 600px; margin: 0 auto;">
    <p style="font-family: monospace; font-size: 11px; letter-spacing: 0.15em; text-transform: uppercase; color: #6b6560; margin-bottom: 32px;">
      Automated Agile · Process Architecture
    </p>
    <h1 style="font-size: 28px; font-weight: 400; color: #e8e0d4; margin-bottom: 12px; line-height: 1.2;">
      Your document is attached
    </h1>
    <p style="color: #8a8278; font-size: 15px; line-height: 1.7; margin-bottom: 32px;">
      The complete Automated Agile process architecture is attached as a Markdown (.md) file. 
      It contains all six sections — the Fundamental Model, Decision Taxonomy, Meeting Architecture, 
      Self-Curating Context Model, Integration Architecture, and Sign-Off Matrix.
    </p>
    <p style="color: #8a8278; font-size: 13px; line-height: 1.7; margin-bottom: 8px;">
      You can open it in any Markdown editor, paste it into Notion, Obsidian, GitHub, or any tool that reads .md files.
    </p>
    <hr style="border: none; border-top: 1px solid #1f1f1f; margin: 40px 0;" />
    <p style="font-family: monospace; font-size: 11px; color: #4a4540;">
      contextengineering.team · softwaremanufacturing.team · Paul@AutomatedAgile.co.uk
    </p>
  </div>
</body>
</html>
`;
    ...
    const attachmentBytes = encoder.encode(MARKDOWN_CONTENT);
    const base64Content = btoa(String.fromCharCode(...attachmentBytes));
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'Automated Agile <docs@automatedagile.co.uk>',
        to: [email],
        subject: 'Automated Agile — Process Architecture (Markdown)',
        html: emailBody,
        attachments: [
          {
            filename: 'automated-agile-process-architecture.md',
    ...
    console.error('Error in send-markdown function:', error);
